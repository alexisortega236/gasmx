...
class ImportFuelPrices extends Command
{
    ...
    if ($isFullImport && ! $this->option('confirm-full')) {
        ...
        $this->line('Ejemplo: php artisan fuel:import-prices --all --confirm-full');

        return self::FAILURE;
    }

    $limit = $isFullImport ? null : max(1, (int) $this->option('limit'));
    ...
    try {
        ...
        if ($existingImport !== null && ! $this->option('force')) {
            $this->warn('Este archivo de precios ya fue procesado anteriormente.');
            $this->line("Importación anterior: #{$existingImport->id}");
            $this->line('No se creó una nueva auditoría ni se insertaron datos.');
            $this->line('Usa --force únicamente si quieres reprocesarlo de forma intencional.');
        ...
        /*
         * Solo después de confirmar que el archivo es nuevo
        ...
        if ($placesXml === false || $pricesXml === false) {
            throw new \RuntimeException('No se pudo interpretar uno de los XML descargados.');
        }
        ...
        foreach ($pricesXml->place as $pricePlace) {
            ...
            foreach ($pricePlace->gas_price as $gasPrice) {
                ...
                if (
                    in_array($fuelType, ['regular', 'premium', 'diesel'], true) &&
                    $priceValue > 0
                ) {
        ...
        $this->line("Precios sin coincidencia/omitidos: {$pricesSkipped}");
    ...
    private function download(string $url)
    {
        ...
        if ($response->body() === '') {
            throw new \RuntimeException("La descarga de {$url} llegó vacía.");
        }
